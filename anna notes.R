...
### END :) -------------------------------------------------------------
#Rearrange do pairs are next to each other
summary_table <- sim_images_h0_df %>%
  group_by(contest_id) %>%
  summarise(broodlossexp = ifelse(previous_experience==0, NA, brood_loss)
            , broodlossnaive = ifelse(previous_experience==1, brood_loss, NA)
  )
